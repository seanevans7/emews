...
obslist <- split(x=pdatM, f=pdatM$ID) # M only 
map(obslist, .f=nrow)
class(obslist)
length(obslist)
...
stateNames <- c("haulout", "surface", "shallow", "epipelagic", "benthic")
N <- length(stateNames)

### DEFINE START VALUES

# dive duration (seconds) - all N states have duration: N=5 values
dd.mu0 <- c(12475, 2056, 136, 660, 1094)
dd.sigma0 <- c(15978, 2686, 126, 380, 251)

# hunting depth (metres) - N-2 states have hunting depth: N-2=3 values
hd.mu0 <- c(10, 90, 412)
hd.sigma0 <- c(4, 71, 80)

# proportion of dive duration spent hunting (range 0-1) - only states 3 to N have hunting time: N-2=3 values
ph.mu0 <- c(0.53, 0.46, 0.42)
ph.sigma0 <- c(0.03, 0.04, 0.02)

# proportion of bathymetry reached (hunt_avgdep/bathymetry range 0-1) - only states 3 to N have prop_hunt_bathy: N-2=3 values
pb.pi0 <- c(0.01, 0.01, 0.91)

# salinity at hunting depth - only states 3 to N have salinity: N-2=3 values
sal.mu0 <- c(34.23, 34.36, 34.59)
sal.sigma0 <- c(0.15, 0.10, 0.08)
...
ct70_5st_hmm7Rcpp_M_hourwk <- mod
...
names(viterbi_list_ct70) <- unique(pdatM$ID)

viterbi_vec_ct70 <- unlist(viterbi_list_ct70)
length(viterbi_vec_ct70)
dim(pdat)
pdatM$viterbi_state <- viterbi_vec_ct70

# look at the distribution of Viterbi-decoded states
table(pdatM$viterbi_state)
table(pdatM$ID, pdatM$viterbi_state)
# look at the individual variability in the state occupancy
indivM_tab <- round(table(pdatM$ID, pdatM$viterbi_state)/rowSums(table(pdatM$ID, pdatM$viterbi_state)), digits=3); indivM_tab
range(indivM_tab[,1])

# find average state occupancy
delta.avg <- as.vector(round(table(pdatM$viterbi_state)/length(pdatM$viterbi_state), digits=2)); delta.avg
...
x <- seq(min(pdatM$DURATION), max(pdatM$DURATION), length=1000)
mod[1:2]
mu1_dd <- mod$dd.mu[1]
...
dmarg_dd <- d1_dd + d2_dd + d3_dd + d4_dd + d5_dd 

dd_dat <- pdatM$DURATION
...
ggplot(data=data.frame(x=dd_dat), aes(x,..density..)) + 
  ...
  geom_line(data=data.frame(x=x, d5_md=d5_dd), aes(x, d5_dd, colour="Benthic dive"), size=1.3) +
...
x <- seq(min(pdatM$hunt_avgdep), max(pdatM$hunt_avgdep), length=1000)
mod[3:4]
mu3_hd <- mod$hd.mu[1]
...
ggplot(data=data.frame(x=hd_dat), aes(x,..density..)) + 
  ...
  geom_line(data=data.frame(x=x, d5_md=d5_hd), aes(x, d5_hd, colour="Benthic dive"), size=1.3) +
...
x <- seq(min(pdatM$hunt_prop[pdatM$source=="dv"]),  max(pdatM$hunt_prop), length=1000) 
mod[5:8]
...
dmarg_ph <- d3_ph + d4_ph + d5_ph 

ph_dat <- pdatM %>% filter(source=="dv") %>% dplyr::select(hunt_prop) 
...
ggplot(data=data.frame(x=ph_dat), aes(x,..density..)) + 
  ...
  geom_line(data=data.frame(x=x, d5_ph=d5_ph), aes(x, d5_ph, colour="Benthic dive"), size=1.3) +
  ...
  scale_colour_manual(name="Densities", values = c("Shallow dive" = brew.cols[5], "Epipelagic dive" = brew.cols[1],
                                                   "Benthic dive" = brew.cols[4], 
                                                   "Marginal" = brew.cols[2]),
                      breaks=c("Shallow dive", "Epipelagic dive", "Benthic dive", "Marginal")) +
  scale_linetype_manual(name="Densities", values=c("Shallow dive"=1, "Epipelagic dive"=1, 
...
x <- seq(min(pdatM$psal), max(pdatM$psal), length=1000)
mod[8:9]
mu3_ps <- mod$sal.mu[1]
...
dmarg_ps <- d3_ps + d4_ps + d5_ps 

ps_dat <- pdatM$psal

quartz()
ggplot(data=data.frame(x=ps_dat), aes(x,..density..)) + 
  geom_histogram(boundary=0, binwidth=0.09, fill="grey90") + 
  ylim(0,3) + theme_bw() + 
  ...
  geom_line(data=data.frame(x=x, d5_md=d5_ps), aes(x, d5_ps, colour="Benthic dive"), size=1.3) +
